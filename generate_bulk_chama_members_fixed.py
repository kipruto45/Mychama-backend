...
Fixed version that properly creates ContributionType instances.
Usage: python manage.py shell < generate_bulk_chama_members_fixed.py
"""
...
from apps.finance.models import (
    Wallet, LedgerEntry, JournalEntry, Contribution, Loan, LoanStatus,
    ContributionType, ContributionFrequency
)
...
print("\n" + "=" * 120)
print("GENERATING 1000 MEMBERS FOR YANGU CHAMA (FIXED VERSION)")
print("=" * 120)
...
# Create or get ContributionTypes for this chama
print("\n📝 Setting up ContributionTypes...")
contribution_type_names = ["Share", "Savings", "Fine"]
contribution_types = {}

for name in contribution_type_names:
    ...
    contribution_types[name.lower()] = contrib_type
    status = "created" if created else "existing"
    print(f"   ✅ {name} ({status})")

# Generate phone numbers (start from +25471500000)
...
# Get existing membership user IDs for this chama
existing_member_ids = set(
    Membership.objects.filter(chama=yangu_chama).values_list('user_id', flat=True)
)

# Filter out users who are already members
memberships_to_create_filtered = [
    m for m in memberships_to_create 
    if m.user_id not in existing_member_ids
]

print(f"   Skipping {len(memberships_to_create) - len(memberships_to_create_filtered)} users already in chama")
print(f"   Creating {len(memberships_to_create_filtered)} new memberships...")

if memberships_to_create_filtered:
    Membership.objects.bulk_create(memberships_to_create_filtered, batch_size=100)
    print(f"   ✅ Created {len(memberships_to_create_filtered)} memberships")
...
contribution_type_list = list(contribution_types.values())

for user in created_users[::2]:  # Every other user
    for _ in range(random.randint(1, 3)):  # 1-3 contributions per user
        # Generate receipt code
        receipt_code = f"REC{user.id}_{timezone.now().timestamp()}_{random.randint(1000, 9999)}"
        
        contribution = Contribution(
            chama=yangu_chama,
            member=user,
            contribution_type=random.choice(contribution_type_list),  # Use actual ContributionType instance
            amount=Decimal(random.uniform(500, 5000)).quantize(Decimal('0.01')),
            date_paid=timezone.now().date() - timedelta(days=random.randint(1, 60)),
...
total_contributions = Contribution.objects.filter(chama=yangu_chama, member__in=created_users).count()
...
print(f"\n💳 LOAN STATISTICS:")
total_loans = Loan.objects.filter(chama=yangu_chama, member__in=created_users).count()
