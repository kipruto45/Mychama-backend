...
class IssueEditPageView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        issue.priority = _priority_from_ui(request.POST.get("priority") or issue.priority)
        issue.updated_by = request.user
        issue.save(update_fields=["title", "description", "category", "priority", "updated_by", "updated_at"])
...
class IssueAdminBoardPageView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["in_review_issues"] = queryset.filter(status=IssueStatus.IN_REVIEW)[:50]
