...
def _requires_upgrade_approval(charge_amount: Decimal, rule: BillingRule) -> bool:
    threshold = Decimal(str(rule.upgrade_approval_threshold or "0.00"))
    if threshold <= 0:
        return False
    return charge_amount >= threshold
...
def calculate_prorated_charge(chama, new_plan: Plan, billing_cycle: str) -> dict:
    ...
    if not current_subscription or current_subscription.plan.code == Plan.FREE:
        return {
            ...
            'prorated': False,
            'requires_approval': _requires_upgrade_approval(full_amount, rule),
        }
    ...
    return {
        ...
        'requires_approval': _requires_upgrade_approval(charge_amount, rule),
    }
