...
class PaymentIntentSerializer(serializers.ModelSerializer):
    ...
    def get_masked_phone(self, obj) -> str:
...
class WithdrawalRequestSerializer(serializers.Serializer, _PurposeChoiceMixin):
    ...
    beneficiary_name = serializers.CharField(max_length=160, required=False, allow_blank=True)
    beneficiary_phone = serializers.CharField(max_length=16, required=False, allow_blank=True)
...
class PayoutSendSerializer(serializers.Serializer):
    chama_id = serializers.UUIDField(required=False)
    payout_proof = serializers.CharField(required=False, allow_blank=True, max_length=500)
...
class MpesaSTKPushRequestSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        contribution_type_id = attrs.get("contribution_type_id")
        loan_id = attrs.get("loan_id")
        if contribution_type_id and loan_id:
            raise serializers.ValidationError(
                {"detail": "Provide either contribution_type_id or loan_id, not both."}
            )
        if not contribution_type_id and not loan_id:
            raise serializers.ValidationError(
                {"detail": "Provide contribution_type_id for deposits or loan_id for repayment."}
            )
        return attrs
...
class PaymentStatusSerializer(serializers.ModelSerializer):
    ...
    latest_stk_checkout_request_id = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        fields = [
            ...
            "raw_response",
            "metadata",
            "latest_stk_checkout_request_id",
            "transactions",
            "created_at",
            "updated_at",
        ]
    ...
    def get_latest_stk_checkout_request_id(self, obj) -> str | None:
        latest = obj.stk_transactions.order_by("-created_at").first()
        return latest.checkout_request_id if latest else obj.checkout_request_id
