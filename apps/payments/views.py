...
from apps.payments.tasks import (
    payments_process_c2b_confirmation,
    payments_process_c2b_validation,
    payments_process_stk_callback,
)
...
empty_object_serializer = inline_serializer(name="PaymentSchemaObject", fields={})
callback_ack_response = inline_serializer(
    name="MpesaCallbackAckResponse",
    fields={
        "ResultCode": serializers.CharField(),
        "ResultDesc": serializers.CharField(),
    },
)
stk_push_response = inline_serializer(
    name="MpesaStkPushResponse",
    fields={
        "payment_intent": PaymentStatusSerializer(),
        "checkout_request_id": serializers.CharField(),
        "created": serializers.BooleanField(),
    },
)
intent_with_instructions_response = inline_serializer(
    name="PaymentIntentInstructionsResponse",
    fields={
        "intent": PaymentIntentSerializer(),
        "instructions": serializers.JSONField(),
    },
)
intent_with_checkout_response = inline_serializer(
    name="PaymentIntentCheckoutResponse",
    fields={
        "intent": PaymentIntentSerializer(),
        "checkout_request_id": serializers.CharField(),
        "created": serializers.BooleanField(),
        "allocation": serializers.JSONField(required=False),
        "stk_transaction": serializers.CharField(required=False),
    },
)
withdrawal_send_response = inline_serializer(
    name="WithdrawalSendResponse",
    fields={
        "intent_id": serializers.CharField(),
        "originator_conversation_id": serializers.CharField(allow_blank=True),
        "status": serializers.CharField(),
    },
)
ussd_callback_response = inline_serializer(
    name="UssdCallbackResponse",
...
class InitiateMpesaPaymentView(ChamaScopeMixin, APIView):
    ...
    serializer_class = InitiateMpesaSerializer

    @extend_schema(
        tags=["Payments"],
        operation_id="initiate_mpesa_payment",
        request=InitiateMpesaSerializer,
        responses={
            200: MpesaTransactionSerializer,
            201: MpesaTransactionSerializer,
            402: error_response_serializer(name="InitiateMpesaPaymentRequiredError"),
            502: error_response_serializer(name="InitiateMpesaPaymentGatewayError"),
        },
    )
...
class MpesaCallbackView(APIView):
    ...
    @extend_schema(
        tags=["Payments"],
        operation_id="process_legacy_mpesa_callback",
        request=MpesaCallbackSerializer,
        responses={200: callback_ack_response, 403: error_response_serializer(name="MpesaCallbackAuthError")},
    )
    def post(self, request, *args, **kwargs):
        raw_payload = b""
        try:
...
class MpesaCallbackLogListView(ChamaScopeMixin, generics.ListAPIView):
    ...
    def get_queryset(self):
        if getattr(self, "swagger_fake_view", False):
            return MpesaCallbackLog.objects.none()
...
class PaymentBaseView(APIView):
    ...
    serializer_class = empty_object_serializer

    def dispatch(self, request, *args, **kwargs):
        # Compliance gate: block payment/withdrawal mutations until KYC is approved.
...
class MpesaSTKPushAPIView(PaymentBaseView):
    throttle_classes = [PaymentInitiationRateThrottle]
    serializer_class = MpesaSTKPushRequestSerializer

    @extend_schema(
        tags=["Payments"],
        operation_id="create_mpesa_stk_push",
        request=MpesaSTKPushRequestSerializer,
        responses={200: stk_push_response, 201: stk_push_response},
    )
    def post(self, request):
                ...
                outcome = PaymentWorkflowService.initiate_deposit_stk(
                    {
                        ...
                        "amount": payload["amount"],
                        "phone": payload.get("phone"),
                        "reference_id": payload["contribution_type_id"],
        ...
        return Response(
            {
                "payment_intent": PaymentStatusSerializer(intent).data,
...
class PaymentStatusView(PaymentBaseView):
    serializer_class = PaymentStatusSerializer

    @extend_schema(
        tags=["Payments"],
        operation_id="retrieve_payment_intent_status",
        responses={200: PaymentStatusSerializer},
    )
    def get(self, request, id):
        try:
            intent = PaymentWorkflowService.get_payment_status(intent_id=id, actor=request.user)
        except Exception as exc:  # noqa: BLE001
            return self._handle_error(exc)
        return Response(PaymentStatusSerializer(intent).data)


class MpesaReconcileView(PaymentBaseView):
    serializer_class = ReconcilePaymentSerializer

    @extend_schema(
        tags=["Payments"],
        operation_id="reconcile_mpesa_payment_intent",
        request=ReconcilePaymentSerializer,
    ...
    def post(self, request):
        ...
        if chama_id:
            ...
            if not request.user.is_superuser and effective_role not in {
                MembershipRole.CHAMA_ADMIN,
                MembershipRole.TREASURER,
                MembershipRole.AUDITOR,
            }:
                return Response(
                    {"detail": "Only admin, treasurer, or auditor can reconcile payments."},
                    status=status.HTTP_403_FORBIDDEN,
                )
        try:
            result = PaymentWorkflowService.reconcile_payment_intent(
                intent_id=serializer.validated_data.get("payment_intent_id"),
                chama_id=serializer.validated_data.get("chama_id"),
                actor=request.user,
            )
        except Exception as exc:  # noqa: BLE001
...
class DepositSTKInitiateView(PaymentBaseView):
    throttle_classes = [PaymentInitiationRateThrottle]
    serializer_class = DepositSTKInitiateSerializer

    @extend_schema(
        tags=["Payments"],
        operation_id="create_deposit_stk_intent",
        request=DepositSTKInitiateSerializer,
        responses={200: intent_with_checkout_response, 201: intent_with_checkout_response},
    )
    def post(self, request):
...
class DepositC2BIntentView(PaymentBaseView):
    throttle_classes = [PaymentInitiationRateThrottle]
    serializer_class = DepositC2BIntentSerializer

    @extend_schema(
        tags=["Payments"],
        operation_id="create_deposit_c2b_intent",
        request=DepositC2BIntentSerializer,
        responses={201: intent_with_instructions_response},
    )
    def post(self, request):
...
class SplitPaymentSTKInitiateView(PaymentBaseView):
    throttle_classes = [PaymentInitiationRateThrottle]
    serializer_class = SplitPaymentSTKInitiateSerializer
...
class SplitPaymentC2BIntentView(PaymentBaseView):
    throttle_classes = [PaymentInitiationRateThrottle]
    serializer_class = SplitPaymentC2BIntentSerializer
...
class PaymentAllocationRuleView(PaymentBaseView):
    serializer_class = PaymentAllocationRuleUpsertSerializer
...
class UssdCallbackView(APIView):
    ...
    authentication_classes = []
    throttle_classes = [PaymentInitiationRateThrottle]
    serializer_class = UssdCallbackSerializer
    ...
    @extend_schema(
        tags=["Payments"],
        operation_id="process_ussd_callback",
        request=UssdCallbackSerializer,
        responses={200: ussd_callback_response},
    )
    def post(self, request):
...
class LoanRepaymentSTKInitiateView(PaymentBaseView):
    throttle_classes = [PaymentInitiationRateThrottle]
    serializer_class = LoanRepaymentSTKInitiateSerializer
...
class LoanRepaymentC2BIntentView(PaymentBaseView):
    throttle_classes = [PaymentInitiationRateThrottle]
    serializer_class = LoanRepaymentC2BIntentSerializer
...
class LoanRepaymentStatusView(PaymentBaseView):
    serializer_class = LoanRepaymentStatusQuerySerializer
...
class WithdrawalApproveView(PaymentBaseView):
    serializer_class = IntentApprovalSerializer
...
class WithdrawalSendView(PaymentBaseView):
    serializer_class = PayoutSendSerializer
    def post(self, request, intent_id):
        serializer = PayoutSendSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        try:
            payout = PaymentWorkflowService.send_b2c_payout(
                intent_id=intent_id,
                actor=request.user,
                payout_proof=serializer.validated_data.get("payout_proof", ""),
            )
        except Exception as exc:  # noqa: BLE001
            return self._handle_error(exc)
        return Response(
...
class WithdrawalRejectView(PaymentBaseView):
    serializer_class = IntentApprovalSerializer
    def post(self, request, intent_id):
        serializer = IntentApprovalSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        try:
            intent = PaymentWorkflowService.reject_withdrawal_intent(
                intent_id=intent_id,
                actor=request.user,
                note=serializer.validated_data.get("note", ""),
            )
...
class PendingLoanDisbursementListView(PaymentBaseView):
    serializer_class = TransactionsQuerySerializer
    def get(self, request):
...
class PaymentDisputeListCreateView(PaymentBaseView):
    def get_serializer_class(self):
        ...
        return TransactionsQuerySerializer
    def get(self, request):
        serializer = TransactionsQuerySerializer(data=request.query_params)
        serializer.is_valid(raise_exception=True)
        chama = get_object_or_404(Chama, id=serializer.validated_data["chama_id"])
...
class PaymentDisputeResolveView(PaymentBaseView):
    serializer_class = PaymentDisputeResolveSerializer
...
class AdminTransactionsView(PaymentBaseView):
    serializer_class = AdminTransactionsQuerySerializer
    def get(self, request):
...
class PaymentIntentActivityLogView(PaymentBaseView):
    serializer_class = PaymentActivityLogSerializer
...
class ReconciliationRunsView(PaymentBaseView):
    serializer_class = ReconciliationRunsQuerySerializer
    def get(self, request):
...
class AdminRegisterC2BUrlsView(APIView):
    ...
    @extend_schema(
        tags=["Payments"],
        operation_id="register_mpesa_c2b_urls",
        responses={200: OpenApiTypes.OBJECT},
    )
    def post(self, request):
...
class C2BValidationCallbackView(CallbackBaseView):
    ...
    serializer_class = C2BValidationPayloadSerializer

    @extend_schema(
        tags=["Payments"],
        operation_id="process_mpesa_c2b_validation_callback",
        request=C2BValidationPayloadSerializer,
        responses={200: callback_ack_response},
    )
    def post(self, request):
        ...
        if getattr(settings, "PAYMENTS_CALLBACK_ASYNC", False):
            payments_process_c2b_validation.delay(
                request.data,
                source_ip=source_ip,
                headers={k: v for k, v in request.headers.items()},
            )
...
class C2BConfirmationCallbackView(CallbackBaseView):
    ...
    serializer_class = C2BConfirmationPayloadSerializer

    @extend_schema(
        tags=["Payments"],
        operation_id="process_mpesa_c2b_confirmation_callback",
        request=C2BConfirmationPayloadSerializer,
        responses={200: callback_ack_response},
    )
    def post(self, request):
        ...
        if getattr(settings, "PAYMENTS_CALLBACK_ASYNC", False):
            payments_process_c2b_confirmation.delay(
                request.data,
                source_ip=source_ip,
                headers={k: v for k, v in request.headers.items()},
            )
...
class STKCallbackV2View(CallbackBaseView):
    ...
    @extend_schema(
        tags=["Payments"],
        operation_id="process_mpesa_stk_callback_v2",
        request=MpesaCallbackSerializer,
        responses={200: callback_ack_response, 403: error_response_serializer(name="MpesaStkCallbackError")},
    )
    def post(self, request):
        ...
        if getattr(settings, "PAYMENTS_CALLBACK_ASYNC", False):
            payments_process_stk_callback.delay(
                request.data,
                source_ip=source_ip,
                headers={k: v for k, v in request.headers.items()},
            )
...
class B2CTimeoutCallbackView(CallbackBaseView):
    ...
    serializer_class = B2CTimeoutPayloadSerializer

    @extend_schema(
        tags=["Payments"],
        operation_id="process_mpesa_b2c_timeout_callback",
        request=B2CTimeoutPayloadSerializer,
        responses={200: callback_ack_response},
    )
    def post(self, request):
