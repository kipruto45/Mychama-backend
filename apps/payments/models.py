...
class PaymentIntent(BaseModel):
    ...
    intent_type = models.CharField(max_length=30, choices=PaymentIntentType.choices)
    ...
    def __init__(self, *args, **kwargs):
        # When Django hydrates from DB it passes positional args; avoid mutating kwargs.
        if args:
            super().__init__(*args, **kwargs)
            return

        # Legacy compatibility for older scripts/tests.
        legacy_member = kwargs.pop("member", None)
        legacy_type = kwargs.pop("type", None)

        if legacy_member is not None and "created_by" not in kwargs:
            kwargs["created_by"] = legacy_member

        if legacy_type and "intent_type" not in kwargs:
            type_key = str(legacy_type).strip().lower()
            kwargs["intent_type"] = {
                "deposit": PaymentIntentType.DEPOSIT,
                "withdrawal": PaymentIntentType.WITHDRAWAL,
                "loan_repayment": PaymentIntentType.LOAN_REPAYMENT,
                "loan_disbursement": PaymentIntentType.LOAN_DISBURSEMENT,
            }.get(type_key, legacy_type)

        if "purpose" not in kwargs:
            inferred_intent = str(kwargs.get("intent_type") or "").upper()
            kwargs["purpose"] = {
                PaymentIntentType.DEPOSIT: PaymentPurpose.CONTRIBUTION,
                PaymentIntentType.WITHDRAWAL: PaymentPurpose.OTHER,
                PaymentIntentType.LOAN_REPAYMENT: PaymentPurpose.LOAN_REPAYMENT,
                PaymentIntentType.LOAN_DISBURSEMENT: PaymentPurpose.OTHER,
            }.get(inferred_intent, PaymentPurpose.OTHER)

        if "phone" not in kwargs and legacy_member is not None:
            kwargs["phone"] = getattr(legacy_member, "phone", "")

        if "idempotency_key" not in kwargs:
            kwargs["idempotency_key"] = f"legacy:{uuid.uuid4().hex}"
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["chama", "status", "intent_type"]),
    ...
    def __str__(self) -> str:
        return f"{self.intent_type} {self.amount} {self.status}"
