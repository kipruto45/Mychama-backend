...
class KnowledgeBaseService:
    ...
    def reindex_document(*, document: KnowledgeDocument, actor=None) -> int:
        ...
        created = 0
        for position, (chunk_text, vector) in enumerate(zip(chunks, vectors)):
...
class AIGatewayService:
    ...
    def _detect_tools(*, message: str, mode: str):
        ...
        if "loan" in text or "owe" in text or "balance" in text:
    ...
    def _build_answer(
        ...
        lines = ["Based on your records, here is what I found:"]
        citations: list[dict[str, Any]] = []

        for run in tool_runs:
            lines.append(f"- Tool `{run.tool_name}` executed.")
            result = run.result
            if isinstance(result, dict):
                if "count" in result:
                    lines.append(f"  Count: {result['count']}")
                if "total_contributed" in result:
                    lines.append(
                        f"  Total contributed: KES {result['total_contributed']}"
                    )
                if "next_installment" in result and result["next_installment"]:
                    nxt = result["next_installment"]
                    lines.append(
                        f"  Next installment: KES {nxt['expected_amount']} due {nxt['due_date']}"
                    )
                    citations.append(
                        {
                            "type": "installment",
                            "id": nxt.get("installment_id"),
                        }
                    )
                if "items" in result and isinstance(result["items"], list):
                    for item in result["items"][:3]:
                        citation_id = (
                            ...
                            or item.get("payout_id")
                            or item.get("chunk_id")
                        )
                        if citation_id:
                            citations.append({"type": run.tool_name, "id": citation_id})

        if len(lines) == 1:
            lines.append("- I could not find matching records for your request.")
    ...
    def chat(*, user, chama_id, mode: str, message: str, conversation_id=None):
        ...
        if not runs:
            ...
            for name, args in tool_plan:
                ...
                runs.append(ToolExecution(tool_name=name, args=args, result=result))

        for run in runs:
            AIGatewayService._log_tool_call(
                chama_id=chama_id,
                actor=user,
                name=run.tool_name,
                args=run.args,
            ...
            AIMessage.objects.create(
                ...
                tool_payload={"args": run.args, "result": run.result},
                created_by=user,
                updated_by=user,
            )

        synthesized_answer, citations = AIGatewayService._build_answer(runs)
        if not answer:
        ...
        assistant_message = AIMessage.objects.create(
            ...
            content=answer,
            tool_payload={"citations": citations},
            created_by=user,
            updated_by=user,
        )
