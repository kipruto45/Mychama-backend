...
from apps.finance.models import LedgerEntry, LoanStatus
...
class FraudEngine:
    ...
    def check_stk_failures(
        ...
        user_failures = failed_payments.filter(
            Q(user=user) | Q(phone=user.phone)
        ).count()
    ...
    def check_unusual_loan_pattern(
        ...
        joined_on = membership.joined_at.date() if membership.joined_at else None
        if joined_on:
            days_since_join = (timezone.now().date() - joined_on).days
            if days_since_join < 7:
                # Check for loan applications
                recent_loans = Loan.objects.filter(
                    member=user,
                    chama=chama,
                    created_at__date__gte=joined_on,
                ).count()
        ...
        pending_loans = Loan.objects.filter(
            member=user,
            chama=chama,
            status__in=[LoanStatus.REQUESTED, LoanStatus.REVIEW, LoanStatus.APPROVED, LoanStatus.ACTIVE],
        ).count()
