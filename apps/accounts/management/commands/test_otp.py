...
class Command(BaseCommand):
    ...
    def _test_generate_otp(self, phone: str, user: User, purpose: str):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"   ✓ OTP generated successfully"))
    ...
    def _test_verify_correct_otp(self, phone: str, purpose: str):
        ...
        if success:
            self.stdout.write(self.style.SUCCESS(f"   ✓ OTP verified successfully"))
        ...
        try:
            ...
            self._last_plain_code = plain_code
            self._last_otp_token = otp_token
            self.stdout.write(self.style.SUCCESS(f"   ✓ New OTP generated for lockout test"))
    ...
    def _test_lockout_behavior(self, phone: str, user: User, purpose: str):
        ...
        for i, wrong_code in enumerate(wrong_codes[:max_attempts]):
            ...
            if not success:
                ...
                self.stdout.write(self.style.ERROR(f"   - Unexpected success with wrong code!"))
        ...
        if user.is_locked():
            self.stdout.write(self.style.WARNING(f"   ⚠ User is now locked out!"))
            self.stdout.write(f"   - Locked until: {user.locked_until}")
        else:
            self.stdout.write(self.style.SUCCESS(f"   ✓ User not locked (or lockout not triggered)"))
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"   ✓ Rate limiting allows new OTP"))
        except Exception as e:
