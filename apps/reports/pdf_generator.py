...
try:
    # Optional dependency: reportlab is only required when rendering PDFs.
...
DEFAULT_HEADER_BG = colors.HexColor("#1f2937") if colors is not None else None
...
class ReportPDFRenderer:
    ...
    def _require_reportlab() -> None:
        if colors is None:
            raise RuntimeError(
                "PDF report generation requires the optional 'reportlab' dependency."
            )

    @staticmethod
    def _base_document() -> tuple[BytesIO, SimpleDocTemplate, list, dict]:
        ReportPDFRenderer._require_reportlab()
        buffer = BytesIO()
