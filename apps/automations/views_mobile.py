...
def compliance_view(request):
    ...
    paid = sum(s.amount for s in schedules if s.status == 'paid')
    ...
    # Calculate streak
    streak = 0
    recent_schedules = schedules.order_by('-scheduled_date')[:12]
...
def pending_contributions_view(request, chama_id):
    ...
    for s in schedules:
        pending.append({
            ...
            'member_name': s.member.get_full_name() or s.member.phone,
            'amount': float(s.amount),
            'due_date': s.scheduled_date.isoformat(),
            'status': s.status,
            'days_overdue': (
                timezone.now().date() - s.scheduled_date
            ).days if s.status == 'missed' else 0,
        })
...
def loan_eligibility_view(request):
    ...
    # Get chama config
    chama_config = {
        'max_loan_amount': float(getattr(membership.chama, "max_loan_amount", 0) or 500000),
        'min_loan_amount': float(getattr(membership.chama, "min_loan_amount", 0) or 1000),
        'interest_rate': float(getattr(membership.chama, "loan_interest_rate", 0) or 12),
    }
    ...
    total_outstanding = sum(l.total_due for l in active_loans)
    ...
    if on_time < 50:
        ...
        risk_score = 40
        reasons.append("Below average payment history")

    if total_outstanding > 0:
        risk_score += 10
        reasons.append("Existing loan balance")

    max_loan_amount = Decimal(str(chama_config["max_loan_amount"]))
    if risk_score >= 50:
        max_loan_amount = max_loan_amount * Decimal("0.6")
    eligible = risk_score < 60
...
def loan_approval_queue_view(request):
    ...
    for loan in loans:
        queue.append({
            ...
            'member_name': loan.member.get_full_name() or loan.member.phone,
            'amount': float(loan.principal),
            'purpose': loan.purpose,
            'term_months': loan.duration_months,
            'status': loan.status,
            'applied_at': loan.requested_at.isoformat(),
...
def route_loan_view(request, loan_id):
    ...
    risk_score = getattr(loan, "risk_score", 0) or 0
    if risk_score > 70:
        loan.status = 'review'
    else:
...
def overdue_loans_view(request, chama_id):
    ...
    for loan in loans:
        ...
        overdue.append({
            ...
            'member_id': str(loan.member_id),
            'member_name': loan.member.get_full_name() or loan.member.phone,
            'amount_due': float(loan.total_due),
            'next_payment_date': loan.due_date.isoformat(),
...
def send_overdue_reminder_view(request, loan_id):
    ...
    """
    try:
        loan = Loan.objects.select_related('member', 'chama').get(id=loan_id)
    except Loan.DoesNotExist:
        return Response(
            {'error': 'Loan not found'},
            status=status.HTTP_404_NOT_FOUND
    ...
    Notification.objects.create(
        ...
        message=f'Your loan payment for {loan.chama.name} is overdue.',
        chama=loan.chama,
    )
...
def security_alerts_view(request):
    ...
    notifications = Notification.objects.filter(
        chama_id=chama_id,
        type__in=[NotificationType.SECURITY_ALERT, NotificationType.SYSTEM],
    ).order_by('-created_at')[:50]
    
    alerts = []
    for n in notifications:
        if severity and n.priority != severity.lower():
            continue
        alerts.append({
            'id': str(n.id),
            'title': n.subject or "Security Alert",
            'description': n.message,
            'severity': n.priority.lower() if n.priority else 'normal',
            'timestamp': n.created_at.isoformat(),
            'is_read': n.inbox_status == NotificationInboxStatus.READ,
            'action_type': n.type.lower(),
        })
...
def report_suspicious_activity_view(request):
    ...
    Notification.objects.create(
        ...
        subject=f'Suspicious Activity: {activity_type}',
        message=description,
        priority='high',
        chama=chama,
    )
...
def check_withdrawal_anomaly_view(request):
    ...
    anomaly = detect_withdrawal_anomaly(
        ...
        member_avg_withdrawal=Decimal(str(member_avg)),
        chama_avg_withdrawal=Decimal(str(chama_avg)),
        recent_withdrawal_count=count,
    )
