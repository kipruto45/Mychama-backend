...
from apps.chama.models import Chama, ChamaStatus, Membership, MembershipRole
...
@shared_task
def retry_failed_push_deliveries():
    retried = 0
    failed_deliveries = NotificationDelivery.objects.select_related(
        "notification",
    ...
    for delivery in failed_deliveries:
        try:
            ...
            logger.exception(
                "retry_failed_push_deliveries failed delivery=%s notification=%s",
                delivery.id,
...
def dispatch_scheduled_announcements():
    ...
    scheduled_items = ScheduledAnnouncement.objects.filter(
        status=ScheduledAnnouncementStatus.PENDING,
        scheduled_at__lte=now,
    )
    for announcement in scheduled_items:
        announcement.status = ScheduledAnnouncementStatus.SENT
        announcement.executed_at = announcement.executed_at or now
    ...
    broadcasts = BroadcastAnnouncement.objects.filter(
        status=BroadcastAnnouncementStatus.PENDING,
        scheduled_at__isnull=False,
        scheduled_at__lte=now,
    )
    broadcast_marked_sent = 0
    for announcement in broadcasts:
        ...
        broadcast_marked_sent += 1

    return {
        "scheduled_announcements_sent": marked_sent,
        "broadcasts_sent": broadcast_marked_sent,
    }


@shared_task
def remind_unread_high_priority_announcements():
    now = timezone.now()
    ...
    for announcement in broadcasts:
        related_notifications = Notification.objects.filter(
            ...
            send_sms=True,
        ).select_related("recipient", "chama")
        for notification in related_notifications:
            NotificationService.send_notification(
                user=notification.recipient,
                chama=notification.chama,
                channels=["sms"],
                subject=f"Unread: {announcement.title}",
                message=(
                    f"You still have an unread announcement from {notification.chama.name}: "
                    f"{announcement.title}. Open MyChama to review it."
                ),
                notification_type=NotificationType.GENERAL_ANNOUNCEMENT,
                priority=NotificationPriority.HIGH,
                idempotency_key=f"announcement-followup:{announcement.id}:{notification.recipient_id}",
                context_data={
...
def send_push_notification_task(self, notification_id: str):
    ...
    Args:
        notification_id: ID of the Notification object to send push for
    """
    try:
        notification = Notification.objects.get(id=notification_id)
        
        # Only send if enabled and has recipient
        if not notification.recipient or not getattr(settings, 'PUSH_NOTIFICATION_ENABLED', True):
            logger.info(f"Push notification skipped for {notification_id}")
            return {"status": "skipped", "reason": "disabled_or_no_recipient"}
        
        # Send push notification
        NotificationService.send_push_for_notification(notification)
        ...
        logger.exception(f"Error sending push notification for {notification_id}: {exc}")
        # Retry with exponential backoff
        raise self.retry(exc=exc, countdown=60 * (2 ** self.request.retries))


@shared_task(bind=True, max_retries=3)
def send_batch_push_notifications(self, notification_ids: list):
    """
    Send push notifications for multiple notifications.
    ...
    for notification_id in notification_ids:
        try:
            ...
            results.append({"notification_id": notification_id, "status": "queued"})
        except Exception as exc:
            logger.exception(f"Error queuing push notification for {notification_id}")
            results.append({"notification_id": notification_id, "status": "error", "error": str(exc)})
    
    return {"total": len(notification_ids), "results": results}


@shared_task
def clean_expired_device_tokens():
    """
    ...
    logger.info(f"Cleaned up {deleted_count} expired device tokens")
