...
class ScheduledAnnouncementCreateSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        channels = set(attrs.get("channels") or [])
        priority = attrs.get("priority", NotificationPriority.NORMAL)
        if "email" in channels and priority != NotificationPriority.CRITICAL:
            raise serializers.ValidationError(
                {"channels": "Email delivery is only allowed for critical announcements."}
            )
        return attrs
...
class BroadcastAnnouncementCreateSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if (
            target == BroadcastTarget.SPECIFIC
            and not attrs.get("target_member_ids")
            and not attrs.get("segment")
        ):
            raise serializers.ValidationError(
                {
                    "target_member_ids": (
        ...
        if attrs.get("segment") and target != BroadcastTarget.SPECIFIC:
            attrs["target"] = BroadcastTarget.SPECIFIC
        channels = set(attrs.get("channels", []))
        priority = attrs.get("priority", NotificationPriority.NORMAL)
        if NotificationChannel.EMAIL in channels and priority != NotificationPriority.CRITICAL:
            raise serializers.ValidationError(
                {"channels": "Email delivery is only allowed for critical announcements."}
            )
        return attrs
...
class NotificationPreferencePutSerializer(serializers.Serializer):
    ...
    email_contribution_reminders = serializers.BooleanField(required=False)
    email_meeting_notifications = serializers.BooleanField(required=False)
    email_payment_confirmations = serializers.BooleanField(required=False)
    email_loan_updates = serializers.BooleanField(required=False)
    email_general_announcements = serializers.BooleanField(required=False)

    sms_contribution_reminders = serializers.BooleanField(required=False)
    sms_meeting_notifications = serializers.BooleanField(required=False)
    sms_payment_confirmations = serializers.BooleanField(required=False)
    sms_loan_updates = serializers.BooleanField(required=False)
    sms_general_announcements = serializers.BooleanField(required=False)
...
class NotificationBroadcastHistoryFilterSerializer(serializers.Serializer):
    chama_id = serializers.UUIDField(required=False)
    status = serializers.ChoiceField(
        choices=BroadcastAnnouncement._meta.get_field("status").choices,
        required=False,
    )
    channel = serializers.ChoiceField(
        choices=["sms", "email", "in_app", "push"],
        required=False,
    )
...
class AnnouncementFeedQuerySerializer(serializers.Serializer):
    chama_id = serializers.UUIDField(required=False)
...
class AnnouncementFeedItemSerializer(serializers.Serializer):
    id = serializers.UUIDField()
    title = serializers.CharField()
    message = serializers.CharField()
    chama_id = serializers.UUIDField(allow_null=True, required=False)
    chama_name = serializers.CharField(required=False, allow_blank=True)
    created_at = serializers.DateTimeField()
    sent_at = serializers.DateTimeField(allow_null=True, required=False)
    action_url = serializers.CharField(required=False, allow_blank=True)
    priority = serializers.ChoiceField(choices=Notification._meta.get_field("priority").choices)
...
class CommunicationAnalyticsQuerySerializer(serializers.Serializer):
    chama_id = serializers.UUIDField(required=False)
    date_from = serializers.DateTimeField(required=False)
    date_to = serializers.DateTimeField(required=False)


class CommunicationDeliveryLogQuerySerializer(serializers.Serializer):
    chama_id = serializers.UUIDField(required=False)
    channel = serializers.ChoiceField(choices=NotificationChannel.choices, required=False)
    status = serializers.ChoiceField(
        choices=NotificationDelivery._meta.get_field("status").choices,
        required=False,
    )
    priority = serializers.ChoiceField(
        choices=Notification._meta.get_field("priority").choices,
...
class AdminDeliveryLogSerializer(serializers.ModelSerializer):
    ...
    recipient_id = serializers.UUIDField(source="notification.recipient_id", read_only=True)
    recipient_name = serializers.CharField(source="notification.recipient.full_name", read_only=True)
    chama_id = serializers.UUIDField(source="notification.chama_id", read_only=True)
