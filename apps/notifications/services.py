...
class NotificationService:
        ...
        *,
        ...
        try:
            result = send_email_message(
                ...
                html_body=notification.html_message,
            )
            NotificationLog.objects.create(
                notification=notification,
                channel=NotificationChannel.EMAIL,
    ...
    def _resolve_chama(*, chama, user):
        ...
        if not membership:
            raise ValueError("Unable to resolve chama for notification.")
    ...
    def _apply_preferences(
        *,
        ...
        if notification_type == NotificationType.CONTRIBUTION_REMINDER:
            ...
            send_email = send_email and preference.email_general_announcements
...
def notify_loan_approved(loan):
    ...
    return create_notification(
        ...
        title="Loan Approved",
        message=f"Your loan application of KES {loan.amount:,.2f} has been approved.",
...
def notify_loan_rejected(loan, reason: str = ""):
    ...
    msg = f"Your loan application of KES {loan.amount:,.2f} was rejected."
    if reason:
        msg += f" Reason: {reason}"
    return create_notification(
        recipient=loan.created_by,
...
def notify_loan_disbursed(loan):
    ...
    return create_notification(
        ...
        message=f"Your loan of KES {loan.amount:,.2f} has been disbursed to your wallet.",
        priority=NotificationPriority.HIGH,
        category=NotificationCategory.LOANS,
        action_url=f"/member/loans/{loan.id}",
        metadata={
...
def notify_contribution_recorded(contribution):
    ...
    return create_notification(
        ...
        priority=NotificationPriority.NORMAL,
        category=NotificationCategory.PAYMENTS,
        action_url=f"/member/contributions",
        metadata={
