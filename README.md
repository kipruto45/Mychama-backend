...
# - Use DATABASE_URL (or DATABASE_POOL_URL) for Supabase Postgres.
# - Configure Supabase Storage for media/KYC (required when PRIVATE_MEDIA_STORAGE_REQUIRED=True).
SUPABASE_URL=https://<project-ref>.supabase.co
SUPABASE_ANON_KEY=
SUPABASE_SERVICE_ROLE_KEY=
SUPABASE_JWT_SECRET=
SUPABASE_STORAGE_BUCKET=MyChama
...
- **Database migrations**: point `DATABASE_URL` (or `DATABASE_POOL_URL`) to your Supabase Postgres instance, then run `python manage.py migrate`.
- **Storage bucket**: this repo includes a Supabase SQL migration to create the private `MyChama` bucket at `supabase/migrations/20260331212058_setup-mychama-storage-bucket.sql`. Apply it via the Supabase CLI (`supabase db push`) or create the bucket manually in the Supabase dashboard.
