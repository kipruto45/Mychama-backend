...
class LoanEligibilityResult:
    ...
    recommended_terms: Optional[int] = None
    interest_rate_recommendation: Optional[Decimal] = None
...
def detect_withdrawal_anomaly(
    ...
    if amount > chama_avg_withdrawal * Decimal(str(threshold_multiplier * 2)):
        return AnomalyAlert(
            ...
            severity="high",
            description=f"Amount unusually large for this chama",
...
def detect_role_change_anomaly(
    changed_by_role: str,
    target_role: str,
    time_since_last_change: Optional[timedelta],
) -> Optional[AnomalyAlert]:
